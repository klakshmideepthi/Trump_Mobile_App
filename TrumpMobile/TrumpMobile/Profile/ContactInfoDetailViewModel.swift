  ...
  func loadContactInfo() {
    ...
    dispatchGroup.notify(queue: .main) {
      ...
      if let contactError = contactError {
        self.showError(
          message: "Failed to load contact information: \(contactError.localizedDescription)")
      } else if let shippingError = shippingError {
        self.showError(
          message: "Failed to load shipping address: \(shippingError.localizedDescription)")
      }
    }
  }
